...
const MinterSupply = (props) => {
  const [walletAddress, setWallet] = useState("");

  // eslint-disable-next-line no-unused-vars
  const [status, setStatus] = useState("");
  // eslint-disable-next-line no-unused-vars
  const [supply, setSupply] = useState(1001);
  ...
  const contractAddress = "0x637598669C5383e94117959aB84f765B086Aa4FB";
  ...
  const getFreePrice = useCallback(async () => {
    ...
    if (newBalanceOf < "2.0" && totalSupply < supply) {
      setIsFree(true);
    } else {
      setIsFree(false);
    }
